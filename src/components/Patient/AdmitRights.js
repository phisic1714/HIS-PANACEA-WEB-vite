...
export default function AdmitRights({
    patientId = null,
    admitId = null,
    admitRightId = null,
    ...
    const getAdmitRights = async (admitId) => {
        if (!admitId) return resetData()
        const req = {
            patientId: patientId,
            admitId
        }
        let res = await callApis(apis["GetAdmitRight"], req)
        ...
        setOptionsRight(res)
        if (admitRightId) {
            const findByOpdRightId = res.find(o => o.admitRightId === admitRightId)
    ...
    // Effects
    useEffect(() => {
        getAdmitRights(admitId)
    }, [admitId, admitRightId, reload])
    useEffect(() => {
        ...
        <Row gutter={[4, 4]} style={{ flexDirection: "row" }}>
            ...
            <Col span={spanCol2}>
                <Select
                    ...
                    value={selectedRightDetails?.admitRightId}
